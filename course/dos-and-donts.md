...
- **Few-shot примеры** (2-4) для специфичных паттернов
...
- **Надеяться на смену модели** — она не компенсирует плохой промпт
...
- **Правило 3 итераций:** проблема не решена за 3 попытки — меняй подход
...
- Деплой в production
- Бизнес-критичный код (платежи, PII)
...
- **API-ключи, токены, пароли, `.env`** — риск: утечка credentials, доступ к инфраструктуре
- **PII (email, телефоны, адреса)** — риск: GDPR, штраф до 20M EUR
- **Медицинские данные** — риск: HIPAA, штраф до $1.5M/год
- **NDA-документы, конфиденциальный код** — риск: юридическая ответственность
- **Session tokens, JWT, cookies** — риск: захват сессий
- **Внутренние credentials клиентов** — риск: репутационный и юридический ущерб
...
- Доверять агенту на слово — модели галлюцинируют и проявляют sycophancy
