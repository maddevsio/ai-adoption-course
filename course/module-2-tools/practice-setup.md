...
- [ ] macOS 11+, Linux (Ubuntu 20.04+), или Windows 11 с WSL2
- [ ] Доступ к интернету для установки
- [ ] 2 GB свободного места на диске
- [ ] Anthropic API key или Claude.ai аккаунт
...
**Официальный метод (рекомендуется, февраль 2026):**
...
```bash
# Установка через официальный tap
...
**Примечание:** NPM-установка (`npm install -g claude-code`) устарела с февраля 2026 года. Используйте только официальные методы установки выше.

### Вариант 2: Linux

**Официальный метод (универсальный способ):**
...
**Примечание:** Apt-репозитории и npm-установка для Claude Code не поддерживаются с февраля 2026 года. Используйте только официальный установщик выше.
...
# Установка Claude Code через официальный скрипт
curl -fsSL https://claude.ai/install.sh | bash
...
- API key (можно получить бесплатно для тестирования)
...
### Вариант B: Claude Max подписка (для интенсивного использования)

**Требования:**
- Активная подписка Claude Max ($100/мес или $200/мес для Max 20x)
- Аккаунт на claude.ai

**Особенности:**
- Фиксированная стоимость независимо от объема использования
- Приоритетный доступ к моделям
- Более высокие лимиты запросов

**Настройка:**

1. Оформить подписку на https://claude.ai/settings/subscription
2. Использовать тот же метод настройки API key через console.anthropic.com
3. Или использовать OAuth токен из веб-интерфейса claude.ai
...
```bash
# Создать конфигурацию Claude Code
mkdir -p ~/.claude
...
**Важно:** Файл конфигурации поддерживает следующие параметры:
- `model` — модель по умолчанию (например, "claude-sonnet-4-5-20250929")
- `maxTurns` — максимальное количество итераций агента
- Параметр `tools` НЕ поддерживается (все инструменты активны по умолчанию)

**Доступные модели:**
- `claude-opus-4-6` — умная модель для сложных задач ($5-21 за 1M токенов)
- `claude-sonnet-4-5-20250929` — рабочая модель для 90% задач ($1.75-3 за 1M токенов) — **рекомендуется**
- `claude-haiku-4-5` — быстрая модель для простых задач ($0.03-1 за 1M токенов)
