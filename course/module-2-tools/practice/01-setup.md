...
> [!NOTE]
> Время выполнения 2-5 минут — это первый раз. Если агент зависает или запрашивает уточнения, это нормально. Помогите ему, сузив задачу: "проанализируй папку src/" вместо "проанализируй весь проект".
...
<details>
<summary>Шаблон CLAUDE.md (нажмите, чтобы развернуть)</summary>
...
> [!IMPORTANT]
> `CLAUDE.md` читается агентом автоматически. Ошибки или устаревшие правила здесь будут применены ко ВСЕМ его действиям. Обновляйте этот файл по мере роста проекта.
...
> [!CAUTION]
> Обязательно заблокируйте `.env` и файлы с секретами. Агент может случайно вывести их содержимое в контекст или включить в коммит.

**Рекомендуемые настройки безопасности:**
- **`blockedPaths`** — файлы и директории, которые агент не должен читать или изменять
  - Обязательно: `.env`, файлы с секретами, приватные ключи
...
[← 3. Модели и мульти-модельная стратегия](../theory/03-multi-model.md) | [Оглавление](../../../README.md) | [Troubleshooting: Решение типовых проблем →](02-troubleshooting.md)
