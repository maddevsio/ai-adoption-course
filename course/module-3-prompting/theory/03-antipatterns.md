...
```
Оптимизируй функцию calculateStats() для массивов 100K+ элементов.
Текущая проблема: O(n²) из-за вложенных циклов на строках 45-52.
Цель: снизить до O(n log n) или O(n).
Можно использовать дополнительную память O(n).
Сохрани сигнатуру функции.
Покрой изменения тестами с массивами 1K, 10K, 100K элементов.
```

> [!TIP]
> Если лень писать детальный промпт — используйте skills (`/writing-plans`, `/debugging` и прочие). Они заставляют агента задавать уточняющие вопросы. Это экономит время лучше, чем угадывать в спешке.
...
Агент попытается сделать всё сразу. Результат — поверхностная реализация, пропущенные детали, несогласованные компоненты.
...
Агент не знает: язык, фреймворк, хранилище пользователей, схему БД, механизм аутентификации, библиотеки проекта. Результат несовместим с кодовой базой.

**Хорошо**:

```
В проекте на NestJS (TypeScript) с TypeORM и PostgreSQL
реализуй метод для аутентификации.

- Сущность User (entities/user.entity.ts), поля: email, passwordHash
- Пароли хешируются bcrypt (cost 12)
- JWT: access token (15 мин) + refresh token (7 дней)
- Добавь метод login(email, password) в AuthService
- Возврат: {accessToken, refreshToken, userId}
- При неверных credentials — throw UnauthorizedException
```
...
Агент реализует что-то, но не может проверить себя. Нет ожидаемого поведения, нет граничных случаев, нет определения "готово". Результат может не соответствовать ожиданиям.
...
> [!IMPORTANT]
> Критерии приёмки — способ для агента проверить себя. Без них он не знает, когда задача выполнена. С ними — может верифицировать каждый пункт и итерировать до соответствия.

---

[← 3. Продвинутые техники промптинга](02-advanced-techniques.md) | [Оглавление](../../../README.md) | [5. Промптинг для разных типов задач →](04-task-specific-prompts.md)
