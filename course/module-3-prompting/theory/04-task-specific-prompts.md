...
<!-- Title: Промптинг для разных типов задач -->
...
```
Напиши API-документацию для WebSocket endpoint /api/v1/notifications.

Включи:
- Описание протокола
- Типы сообщений (subscribe, unsubscribe, notification)
- JSON schema для каждого типа
- Последовательность handshake
- Возможные ошибки (401, 429, 500)

Аудитория — frontend и mobile разработчики без опыта WebSocket.
```
...
```
Нужно спроектировать систему для обработки 50K событий/сек.
Бизнес-задача следующая ...

Требования:
1) Опиши архитектуру
2) Объясни, как обеспечить идемпотентность
3) Обдумай как масштабировать
4) Сравни trade-offs: простота vs производительность vs надёжность

Для каждого решения — аргументы за/против с примерами из практики.
```
