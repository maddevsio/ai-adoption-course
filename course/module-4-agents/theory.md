...
**Примечание о названиях:** В разных инструментах используются разные названия для этого файла: `AGENTS.md`, `CLAUDE.md` (синоним для Claude Code), `.cursorrules` (для Cursor), `.windsurfrules` (для Windsurf). Суть одна — правила работы агента с проектом. В курсе мы используем термин **AGENTS.md** как универсальный.
