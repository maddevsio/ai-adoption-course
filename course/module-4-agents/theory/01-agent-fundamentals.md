...
Вместо "код должен работать" используйте конкретные критерии: 1) все тесты проходят (pytest без ошибок), 2) линтер чист (flake8, mypy), 3) покрытие тестами минимум 80% (coverage report)
...
ИИ снижает стоимость написания тестов. Поэтому покрытие должно быть высоким, а E2E тесты — не привилегия, а необходимость. Подробнее — в [тестирование с агентами](03-testing-with-agents.md).
...
Ключевое разделение в работе с агентами — режимы **Plan** и **Act/Execute**. Это разные режимы мышления с разными требованиями к модели, бюджету токенов, и уровню вашего контроля.
...
**1. Stack**: точные версии, чтобы агент генерировал совместимый код. Не "используем FastAPI", а "FastAPI 0.104.1
