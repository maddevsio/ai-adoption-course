...
Unit тесты дают быстрый feedback loop для агента. Агент написал код → запустил unit тесты → увидел ошибку → исправил → повторил. Если feedback loop медленный (только E2E), агент потратит токены впустую.
...
> [!WARNING]
> Никогда не давайте агенту право менять тесты вместо кода. Если тест падает — ВСЕГДА исправляйте код. Единственное исключение: тест содержит ошибку в самом тесте (неправильное expected value).
