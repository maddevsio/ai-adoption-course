...
<!-- Title: Implementation Notes (Trace): [Название фичи/плана] -->
...
> [!TIP]
> - **Удобство разработчика:** trace должен быть полезен при возвращении к задаче через неделю
...
> - **Timestamps в именах файлов** (`2026-01-15-task-filtering.md`) помогают агенту определить актуальность документа
...
> **Примечание для агентов:** Этот trace должен быть создан **после** завершения реализации. Заполняй инкрементально по ходу работы — не пытайся вспомнить всё в конце. Trace — это field notes, а не отчёт постфактум. Список артефактов доступен через `git diff` — дублировать его в trace не нужно.
