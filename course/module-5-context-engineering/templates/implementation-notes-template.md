...
Файлы, созданные в процессе работы.

**Код:**
- [Список файлов: например, `src/handlers/tasks.go`, `src/services/task_service.go`]

**Тесты:**
- [Список тестовых файлов: например, `src/handlers/tasks_test.go`]

**Миграции:**
- [Список миграций: например, `migrations/003_add_task_filters_index.sql`]

**Документация:**
- [Обновлённые/созданные документы: например, `docs/api.md`]

**Пример:**
> **Код:**
> - `backend/internal/handlers/task_handler.go` (изменён)
> - `backend/internal/services/task_service.go` (изменён)
> - `backend/internal/repositories/task_repository.go` (изменён)
>
> **Тесты:**
> - `backend/internal/handlers/task_handler_test.go` (создан)
> - `backend/internal/services/task_service_test.go` (создан)
>
> **Миграции:**
> - `backend/migrations/003_add_task_filters_index.up.sql` (создан)
> - `backend/migrations/003_add_task_filters_index.down.sql` (создан)
>
> **Документация:**
> - `docs/api.md` (обновлён — добавлен раздел о фильтрации)
...
Полезные ссылки, использованные в процессе работы.

- [Спецификация фичи](../specs/task-filtering.spec.md)
- [Конституция проекта](../AGENTS.md)
- [Документация фреймворка](https://example.com)
- [Stack Overflow thread](https://stackoverflow.com/...)
- [Связанный PR](https://github.com/user/project/pull/123)
...
Ключевые фрагменты кода для справки (опционально).
...
```go
func parseTaskFilters(c echo.Context) (TaskFilters, error) {
    filters := TaskFilters{}

    if status := c.QueryParam("status"); status != "" {
        status = strings.ToLower(status)
        if !isValidStatus(status) {
            return filters, errors.New("invalid status value")
        }
    ...
    if priority := c.QueryParam("priority"); priority != "" {
        ...
        if !isValidPriority(priority) {
            return filters, errors.New("invalid priority value")
        }
...
[Добавьте ключевые фрагменты кода, если нужно для справки]

---

## Approval / Sign-off

**Completed by:** [Имя агента/разработчика]

**Reviewed by:** [Имя ревьюера]
...
> **Примечание для агентов:** Этот trace должен быть создан **после** завершения реализации. Заполняй инкрементально по ходу работы — не пытайся вспомнить всё в конце. Trace — это field notes, а не отчёт постфактум.
