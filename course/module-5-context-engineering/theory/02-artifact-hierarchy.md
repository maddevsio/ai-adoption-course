...
Артефакты — это не просто документы. Это иерархия, где каждый уровень решает свою задачу:
...
Конституция — это **не** AGENTS.md. AGENTS.md — краткая справка (tech stack, команды запуска, базовые правила). 

Конституция — полный документ с ADR, паттернами, антипаттернами (~400-500 строк), который слишком велик для AGENTS.md.
...
Разделение ролей (dev-mode пишет код, reflect-mode пишет constitution) защищает главный артефакт от хаотичных изменений. Агенты-разработчики создают traces с предложениями, reflect-mode принимает финальное решение о том, что попадёт в constitution.
