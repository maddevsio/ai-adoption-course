...
<!-- Title: Plan 003: Authentication System -->
...
- **Constitution** — правила и стандарты проекта. Обновляется в reflect-mode. Читают все агенты перед работой.
- **Plan** — задачи конкретной итерации. Создаётся перед началом работы. Читает агент-исполнитель.
- **Trace** — лог выполнения плана. Заполняется во время и после работы. Читает reflect-mode агент.
- **Fail** — постмортем инцидента. Создаётся после обнаружения проблемы. Читают все агенты.

### 3.1. Constitution (Конституция проекта)

**Что это:** живой документ ~400-500 строк, обязательный к прочтению перед любой работой. Содержит:
- tech stack
- архитектурные решения (ADR)
- обязательные паттерны
- запреты (antipatterns)

**Где хранить:** `constitution.md` в директории документации.

Конституция — не AGENTS.md. AGENTS.md — краткая справка (tech stack, команды запуска). Конституция — полный документ с ADR и паттернами.
