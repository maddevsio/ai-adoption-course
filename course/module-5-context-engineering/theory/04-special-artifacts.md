...
Это "README для агентов" — файл в корне проекта, который агенты читают автоматически. Содержит описание проекта, архитектуры, команды для запуска, конвенции кода.

**Ключевое отличие от README:** AGENTS.md содержит детальный контекст для агентов (build steps, test conventions, паттерны). Этот контекст захламлял бы обычный README.
...
**Skills** — готовые промпты и подходы для конкретных повторяющихся задач. Вместо повторения промпта "сделай code review..." создаёте skill `/review`.

В Claude Code skills — это markdown-файлы с front matter. Хранятся в `~/.claude/skills/` (глобально) или `project/.claude/skills/` (для проекта). Файл всегда называется `SKILL.md`, а имя skill определяется именем папки.
