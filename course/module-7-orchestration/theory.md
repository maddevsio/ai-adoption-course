...
Аналогия из **Enji Fleet** (внутренний проект Mad Devs по созданию платформы оркестрации AI-агентов, используемый в курсе как демонстрация best practices): менеджер ставит задачу → флит агентов автономно декомпозирует её на подзадачи, кодит, тестит, оформляет PR → разработчик ревьюит и мерджит. Разработчик здесь выступает как мейнтейнер флита, решатель блокеров, оптимизатор скорости работы системы.
...
Как отмечает команда **Enji Fleet** (учебный пример паттернов оркестрации): *"Разработчики становятся мейнтейнерами флита, решателями блокеров, оптимизаторами скорости флита и его ресурсов."* Это не потеря профессии — это эволюция от исполнителя к архитектору систем.
...
Правило из **Enji Fleet** (демонстрация best practice): **каждый план = отдельный worktree**. Никогда не работайте в основной директории при параллельной разработке. Агент начинает — создаём worktree. Агент закончил (или упал) — удаляем worktree. Это дисциплина, которая предотвращает 90% проблем с конфликтами.
...
Не стоит использовать один универсальный агент на все задачи. Опыт **Enji Fleet** (демонстрационный проект для курса) показывает: специализированные роли работают эффективнее.
...
**Пример конфигурации из Enji Fleet** (учебный пример — agent-profiles.yaml):
...
Координатор управляет цепочкой: **Research → Plan → Implement → Review**. Координатор не пишет код, а формирует задачи с чёткими критериями приёмки для каждого этапа. (Паттерн из **Enji Fleet** — учебного проекта Mad Devs)
...
**Библия** (концепция из **Enji Fleet** — демонстрационный проект) — это верхнеуровневый ценностный документ, который определяет "кто ты, что ты, для чего ты, кому и как помогаешь". От библии выкристаллизовывается вся спека для флита.
...
**Три типа памяти** (из **Enji Fleet** — учебный пример архитектуры памяти):
