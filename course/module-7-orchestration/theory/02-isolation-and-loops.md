...
Каждый агент работает в своей сессии со своим контекстом. Если два агента одновременно редактируют файлы в одной директории, они перезапишут изменения друг друга или получат конфликты merge.
...
Агент не должен останавливаться при первой ошибке. Человек вовлекается только в двух случаях: агент выполнил задачу или застрял после нескольких итераций.
...
**Fail-until-done** — агент работает в цикле до достижения успеха. Нет понятия "я попробовал и не вышло". Есть понятие "я буду пробовать, пока не получится или не упрусь в hard limit". Примеры лимитов: 50 итераций или 3 часа работы.
