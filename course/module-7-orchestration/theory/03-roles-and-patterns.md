...
- **Архитектор** — дорогая модель (Opus 4.6, GPT-5). Проектирование системы, ADR, выбор паттернов. Когда: начало проекта, новая крупная фича.
- **Планировщик** — средняя модель (Sonnet 4.5). Декомпозиция задач, определение порядка, критерии приёмки. Когда: после проектирования, перед разработкой.
- **Разработчик** — дешёвая модель (Haiku, Codex Mini, Gemini Flash). Написание кода по спецификации, тесты, self-check. Когда: основная масса работы.
- **QA / Ревьюер** — средняя модель (Sonnet). Интеграционное тестирование, проверка edge cases, баг-репорт. Когда: после разработки, перед merge.
...
[← 3. Изоляция контекстов с git worktree](02-isolation-and-loops.md) | [Оглавление](../../../README.md) | [7. "Библия" и Playbook оркестратора →](04-playbook-and-operations.md)
