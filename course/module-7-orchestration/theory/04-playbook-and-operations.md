...
**"Библия"** — надстройка над конституцией. Определяет контекст для флота агентов: целевые пользователи, типы задач, стандарты качества, процессы деплоя. Разные проекты = разные библии.
...
**Пример:** "Агент backend, какой формат ответа у endpoint /api/users? Агент frontend, совпадает ли он с тем, что ты ожидаешь?" Модератор получает оба ответа, находит расхождение и просит агентов согласовать контракт.

### 7.5. Wizard: генерация playbook-а из диалога

Команда `/wizard` генерирует playbook из диалога. Вы описываете задачу в свободной форме. AI задаёт уточняющие вопросы. При достаточной уверенности генерирует готовый playbook с чеклистом.

Документация Maestro: [https://docs.runmaestro.ai/autorun-playbooks](https://docs.runmaestro.ai/autorun-playbooks)
...
- **Время на задачу** (median, p95) — сколько агент тратит на атомарную задачу
- **Количество итераций** до успеха — сколько раз прогонялся цикл Do->Check->Fix
...
3. **Знания по самоулучшению** (lessons -> skills) — агент решил сложный баг и записал lesson. Lesson попадает в constitution. Следующий агент не совершит ту же ошибку.

## 9. Честно про сложности

**Первое время медленнее.** Первые недели уйдут на настройку, отладку, разрешение конфликтов.

**Инфраструктура и стоимость.** Бюджет агентов, credential management, мониторинг -- всё требует настройки. Оплата за токен по API слишком затратна. Берите подписки и бесплатные сервисы, крутите до лимитов.

**Когда оркестратор уместен:**
- **Хорошее тестовое покрытие и CI** — агенты могут автоматически проверять свою работу
- **5+ независимых подзадач** с чёткими критериями приёмки
- **Повторяющиеся задачи** одного типа (миграция 20 сервисов, обновление API в 15 клиентах)
- **Устоявшаяся архитектура** и конвенции, описанные в constitution
- **Команда упирается в пропускную способность** и уже освоила работу с одним агентом
