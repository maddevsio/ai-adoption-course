...
### V0.1 — Карта курса (roadmap)

**Что:** Визуальная карта прогрессии по модулям с указанием уровней зрелости.

**Зачем:** Студент видит весь путь курса, понимает где он сейчас и куда движется. Заменяет текстовое перечисление модулей.

**Как выглядит:** Горизонтальная диаграмма слева направо. Три блока-части (Фундамент, Агентная разработка, Расширение и оркестрация), внутри каждого — модули. Снизу — шкала уровней зрелости (1→5).

**Формат:** Mermaid flowchart LR

```mermaid
flowchart LR
    subgraph part1["Часть I: Фундамент"]
        M0[Модуль 0<br/>Введение]
        M1[Модуль 1<br/>Зачем ИИ]
        M2[Модуль 2<br/>Инструменты]
        M3[Модуль 3<br/>Промптинг]
    end

    subgraph part2["Часть II: Агентная разработка"]
        M4[Модуль 4<br/>Работа с агентами]
        M5[Модуль 5<br/>SDD и контекст]
    end

    subgraph part3["Часть III: Расширение"]
        M6[Модуль 6<br/>MCP]
        M7[Модуль 7<br/>Оркестрация]
    end

    M8[Модуль 8<br/>Ответственность]

    M0 --> M1 --> M2 --> M3 --> M4 --> M5 --> M6 --> M7 --> M8

    L1[Уровень 1-2] -.-> part1
    L3[Уровень 3] -.-> part2
    L45[Уровень 4-5] -.-> part3
```

**Где разместить:** `course/module-0-intro/theory.md`, в секции описания структуры курса

---

## Модуль 1: Зачем разработчику ИИ

### V1.1 — Модель зрелости (5 уровней) — пирамида/лестница

**Что:** Центральная визуализация курса. Пять уровней зрелости ИИ-использования, расположенные как ступени лестницы.

**Зачем:** Это ключевой фреймворк курса, на который ссылаются все модули. Текстовое описание из 5 уровней с характеристиками плохо запоминается. Визуальная лестница создает ментальную модель прогрессии.

**Как выглядит:** Вертикальная лестница снизу вверх. Каждая ступень содержит:
- Номер уровня
- Название роли (чат-помощник, IDE-копилот, водитель агента, специалист, менеджер)
- Ключевой инструмент
- Одно ключевое действие

**Формат:** Mermaid flowchart BT (bottom to top)

```mermaid
flowchart BT
    L1["Уровень 1: Чат-помощник<br/>ChatGPT, Gemini<br/>Спросил → скопировал"]
    L2["Уровень 2: IDE-копилот<br/>GitHub Copilot, Tabnine<br/>Tab → принял подсказку"]
    L3["Уровень 3: Водитель агента<br/>Claude Code, Cursor<br/>Поставил задачу → проверил результат"]
    L4["Уровень 4: Специалист<br/>Агент + MCP + SDD<br/>Написал спеку → агент реализовал"]
    L5["Уровень 5: Менеджер оркестра<br/>Параллельные агенты<br/>Координация потоков работы"]

    L1 --> L2 --> L3 --> L4 --> L5
```

**Где разместить:** `course/module-1-why-ai/theory.md`, в начале секции с описанием модели зрелости
